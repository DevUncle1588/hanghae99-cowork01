...
def tj_card():
    ...
    # 정원 초과시 count값이 올라가지 않는 조건
    if (like_receive == attendance_receive):
        return jsonify({'msg': '인원이 가득 찼습니다.', 'result': 'over'})
    elif (like_receive != attendance_receive):    
        find_writer=db.write_info.find_one({
            'title': title_receive,
            'date': date_receive,
            'place': place_receive,
            'attendance': attendance_receive,
        ...
        find_group_list=find_writer["group_list"]

        for id_list in find_group_list:
            if(id_list==user_id):
                chk_id_list=1
            else:
                chk_id_list=0
        ...
        # 현재 like_receive로 가져운 count값이 str형식이므로 int형으로 변환
            ...
            db.write_info.update_one({
                    ...
                    {'$set': {'count': int_like + 1}, '$push': {'group_list': group_receive}}, 
                upsert=True)
            return jsonify({'msg': '참가신청 되었습니다!'})
        else:
            return jsonify({'msg': '중복신청은 안됩니다!'})
